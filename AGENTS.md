...
| **`1c-configuration-storage`** | Lock / commit / label / unlock in 1C configuration storage; load when `CONFIG_STORAGE_ENABLED=true` for IB/`src/cf` edits or explicit storage ops. |
...
- **configuration-storage** — 1C configuration storage regimen (lock / commit comment+label / never unbind) when `CONFIG_STORAGE_ENABLED=true`. File: `.cursor/rules/configuration-storage.mdc`. Playbook skill: `1c-configuration-storage`. Ask at project init whether storage is used (see `TEMPLATE.md`).
